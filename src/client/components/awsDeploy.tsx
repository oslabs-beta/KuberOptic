...
const AWSDeploy = () => {
  ...
  ipcRenderer.on('clusterClient2', (event: any, arg: any) => {
    ...
    if (!arg.length || !arg || arg === []) {
      ...
      newClusters = Object.values(
        newClusters.reduce(
          (allClusts, nextClust) =>
            Object.assign(allClusts, { [nextClust.clusterName]: nextClust }),
          {},
        ),
      );
      setStore({
  ...
  const awsRegionDisplay = array => {
    ...
    setStore({ ...Store, visualize: false, awsDeployPage: false, deploying: true });
    ipcRenderer.send('asynchronous-message2', arg);
  };
